...
private:
    ...
    BinaryExpressUSBC …qß     
*  ú`  €       üo, bool> operator != ( bool rhs ) {
        return captureExpression<Internal::IsNotEqualTo>( rhs );
    }
...
private:
    template<Internal::Operator Op, typename RhsT>
    BinaryExpressinaryExpression( ResultBuilder& rb, LhsT lhs, RhsT rhs )
...
namespace Catch {
    ...
    public:
        ...
        virtual void registerTest( TestCase const& testCase ) {
            ...
            if( name.empty() ) {
       USBC .Þ     
*  úà  €       À  return getRegistryHub().getTestCaseRegistry().getAllTestsSorted( config );
    }
    ...
    public:
        ...
        virtual void registerTest( TestCase const& testCase ) {
            std::string name = testCase.getTestCaseInfo().name;
            if( name.empty() ) {
       rceNoDuplicateTestCases( m_functions );
...
namespace Catch {
    ...
    Option<TagAlias> TagAliasRegistry::find( std::string const& alias ) const {
        ...
        if( it != m_registry.end() )
            return it->second;
        Spec =  expandedTestSpec.substr( 0, pos ) +
                                    it->second.tag +
    ...
    private:
        ...
        void printTotals( Totals const& totals ) {
            if( totals.testCases.total() == 0 ) {
                ...
                stream << Colour( Colour::ResultSucUSBC …qß     
*  úà  H       (     return *this;
            }
        ...
        void printTotals( Totals const& totals ) {
            if( totals.testCases.total() == 0 ) {
                ...
                stream << Colour( Colour::ResultSuc                         .addRow( totals.assertions.total() ) );
